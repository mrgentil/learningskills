      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        >
          ...
          <p className="text-muted-foreground text-lg">
            Chaque forfait inclut un déploiement en marque blanche et une propriété totale des données.
          </p>
        </motion.div>

        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((pkg, i) => {
            const isHighlighted = pkg.slug === 'pro';
            ...
            return (
              ...
              >
                ...
                <div className="flex items-baseline gap-1 mb-6">
                  <span className="text-3xl font-bold">${Math.floor(pkg.price)}</span>
                  <span className={`text-sm ${isHighlighted ? "text-primary-foreground/70" : "text-muted-foreground"}`}>/mois</span>
                </div>

                <ul className="space-y-3 mb-8 flex-grow">
                  {features.map((f: string, idx: number) => (
                    <li key={idx} className="flex items-center gap-2 text-sm leading-relaxed">
                      <Check className={`w-4 h-4 flex-shrink-0 ${isHighlighted ? "text-accent" : "text-accent"}`} />
                      {f}
                    </li>
                  ))}
                </ul>

                <a href={pkg.id ? `/checkout/${pkg.slug}` : "/register"} className="block mt-auto">
                  <Button
