                    ...
                    @if ($currentLesson->video_url)
                        ...
                        @if ($isYoutube)
                            @php
                                ...
                                // youtu.be/VIDEO_ID (priorité)
                                if (preg_match('%youtu\.be/([a-zA-Z0-9_-]{11})%', $url, $m)) {
                                    $videoId = $m[1];
                                } elseif (preg_match('%(?:youtube\.com/.*[?&]v=|youtube\.com/embed/)([a-zA-Z0-9_-]{11})%', $url, $m)) {
                                    $videoId = $m[1];
                                }
                            @endphp
                            @if($videoId)
                            ...
                            <div class="pm-no-video">
                                ...
                                <p>URL YouTube invalide. Vérifiez le lien dans les paramètres du cours.</p>
                            </div>
                            @endif
                        @elseif($isVimeo)
                            <!-- Vimeo placeholder -->
                        ...
                        .then(data => {
                            if (data.success) {
                                if (data.course_completed) {
                                    Swal.fire({
                                        title: 'Félicitations ! 🎓',
                                        html: '<p class="mb-4">Vous avez terminé ce cours avec succès ! Votre certificat est prêt.</p>' +
                                            (data.certificate_url ? '<a href="' + data.certificate_url + '" class="btn btn-warning btn-lg mt-2" target="_blank"><i class="fa fa-certificate mr-2"></i>Voir / Télécharger mon certificat</a>' : ''),
                                        icon: 'success',
                                    ...
                                    Swal.fire({
                                        ...
                                        html: data.next_lesson_url ?
                                            'Bravo ! Passage à la leçon suivante...' :
                                            'Félicitations !',
                                        icon: 'success',
                                        timer: 2000,
