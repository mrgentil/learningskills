...
class CoursePlayerController extends Controller
{
    ...
    public function show(string $academySlug, string $courseSlug, string $lessonSlug = null)
    {
        ...
        // 1. Load active tenant and set session for correct scope (student viewing academy's content)
    ...
    public function complete(Request $request, $lessonId)
    {
        ...
        // Ensure user is enrolled (active or completed)
        $isEnrolled = $user->enrollments()
        ...
        $certificateUrl = null;
        if ($courseCompleted) {
            $enrollment = $user->enrollments()->with('certificate')->where('course_id', $courseId)->first();
            $cert = $enrollment?->certificate;
            if ($cert) {
                $certificateUrl = route('certificate.show', $cert->certificate_uuid);
            }
        }

        return response()->json([
            'success' => true,
            'message' => $courseCompleted ? 'Félicitations ! Vous avez terminé ce cours.' : 'Leçon marquée comme terminée',
